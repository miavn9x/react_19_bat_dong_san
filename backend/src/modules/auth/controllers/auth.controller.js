 ...
 *  - Logout/Logout-all: revoke phiên & xóa cookie
 *  - Sessions/Delete session
 *  - VIỆT HOÁ thông điệp + log ra terminal khi trả JSON
 */
const crypto = require("crypto");
const {
  createSessionAndTokens,
...
function sendJson(res, status, message, tag = "auth", extra) {
...
async function register(req, res) {
  try {
    const { name, email, password, avatarUrl = "", phone = "", address = "" } = req.body || {};
    if (!name || !email || !password) {
      return sendJson(res, 400, "Thiếu name, email hoặc password", "auth/register");
    }
    ...
    console.log("[auth/register] Đăng ký thành công:", { email: user.email, id: user._id.toString() });
    return res.status(201).json({
      token: accessToken,
      user: {
        _id: user._id, name: user.name, email: user.email,
    ...
    if (err && err.code === 11000) {
      return sendJson(res, 409, "Email đã tồn tại", "auth/register", { email: req.body?.email });
    }
    console.error("[auth/register] Lỗi:", err);
    return sendJson(res, 500, "Lỗi máy chủ", "auth/register");
  }
}
...
async function login(req, res) {
  try {
    const { email, password } = req.body || {};
    if (!email || !password) {
      return sendJson(res, 400, "Thiếu email hoặc mật khẩu", "auth/login");
    }

    const user = await User.findOne({ email }).select("+password");
    if (!user || !(await user.comparePassword(password))) {
      return sendJson(res, 401, "Email hoặc mật khẩu không đúng", "auth/login", {
        email,
        ip: req.ip,
        ua: (req.get("user-agent") || "").slice(0, 64),
      });
    }
    ...
    console.log("[auth/login] Đăng nhập OK:", { email, id: user._id.toString() });
    return res.json({
      token: accessToken,
      user: {
        _id: user._id, name: user.name, email: user.email,
    ...
    return sendJson(res, 500, "Lỗi máy chủ", "auth/login");
  }
}
...
async function refresh(req, res) {
  try {
    ...
    if (!oldToken) return sendJson(res, 401, "Thiếu refresh token", "auth/refresh");
    if (!csrfHdr || !csrfCookie || csrfHdr !== csrfCookie) {
      return sendJson(res, 403, "Mã CSRF không hợp lệ", "auth/refresh");
    }
    ...
    console.log("[auth/refresh] Làm mới token OK:", { userId: user._id.toString() });
    return res.json({
      token: accessToken,
      user: {
        _id: user._id, name: user.name, email: user.email,
    ...
    console.warn("[auth/refresh] Thất bại:", err?.message || err);
    clearRefreshCookie(res);
    return sendJson(res, 401, "Làm mới token không hợp lệ", "auth/refresh");
  }
}
...
async function logout(req, res) {
  try {
    ...
    if (sid && req.userId) await revokeSession(sid, req.userId, "logout");
    clearRefreshCookie(res);
    return sendJson(res, 200, "Đã đăng xuất", "auth/logout", { userId: req.userId, sid });
  } catch (err) {
    console.error("[auth/logout] Lỗi:", err);
    return sendJson(res, 500, "Lỗi máy chủ", "auth/logout");
  }
}

async function logoutAll(req, res) {
  try {
    if (!req.userId) return sendJson(res, 401, "Chưa đăng nhập", "auth/logoutAll");
    await revokeAllSessions(req.userId, "logout_all");
    clearRefreshCookie(res);
    return sendJson(res, 200, "Đã huỷ tất cả phiên đăng nhập", "auth/logoutAll", { userId: req.userId });
  } catch (err) {
    console.error("[auth/logoutAll] Lỗi:", err);
    return sendJson(res, 500, "Lỗi máy chủ", "auth/logoutAll");
  }
}

async function sessions(req, res) {
  try {
    if (!req.userId) return sendJson(res, 401, "Chưa đăng nhập", "auth/sessions");
    const list = await listUserSessions(req.userId);
    return res.json({ items: list });
  } catch (err) {
    console.error("[auth/sessions] Lỗi:", err);
    return sendJson(res, 500, "Lỗi máy chủ", "auth/sessions");
  }
}

async function revoke(req, res) {
  try {
    if (!req.userId) return sendJson(res, 401, "Chưa đăng nhập", "auth/revoke");
    const { sessionId } = req.params;
    await revokeSession(sessionId, req.userId, "revoke_by_user");
    return sendJson(res, 200, "Đã thu hồi phiên", "auth/revoke", { sessionId, userId: req.userId });
  } catch (err) {
    console.error("[auth/revoke] Lỗi:", err);
    return sendJson(res, 500, "Lỗi máy chủ", "auth/revoke");
  }
}
