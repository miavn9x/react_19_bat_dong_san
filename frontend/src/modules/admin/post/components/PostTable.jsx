...
// ===== (tuỳ chọn) highlight đoạn khớp với query =====
function Highlight({ text = "", query = "" }) {
  ...
  const mid = t.slice(idx, idx + q.length); // độ dài gần đúng (không hoàn hảo với tổ hợp dấu)
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    className="px-2.5 py-1.5 rounded-lg border bg-white text-xs hover:bg-gray-50"
                  ...
                  <button
                    className="px-2.5 py-1.5 rounded-lg bg-indigo-600 text-white text-xs hover:bg-indigo-700"
                    onClick={() => onEdit?.(p)}
                  ...
                  <button
                    className="px-2.5 py-1.5 rounded-lg bg-red-600 text-white text-xs hover:bg-red-700"
                    onClick={() => onDelete?.(p)}
                    aria-label="Xoá bài"
                    title="Xoá"
                  >
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p._id} className="border-t">
                ...
                <td className="px-3 py-2 align-top">
                  <Highlight text={p.author?.name || "—"} query={query} />
                </td>
