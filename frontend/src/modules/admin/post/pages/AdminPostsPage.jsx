...
// Debounce nhỏ cho input tìm kiếm
...
/** Modal xác nhận xoá (mobile-first) */
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:w-[520px] bg-white rounded-t-2xl sm:rounded-2xl shadow-lg p-4 sm:p-6">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Bạn sắp xoá bài: <b>{postTitle || "—"}</b>
        </p>

        <div className="mt-4 space-y-3">
          <button
            onClick={onConfirmOnlyPost}
            className="w-full text-left rounded-lg border px-3 py-2 hover:bg-gray-50"
            title="Chỉ xoá bài, giữ nguyên media liên quan"
          >
            <div className="font-medium text-sm">Chỉ xoá bài viết</div>
            <div className="text-xs text-gray-600">
              Bài viết sẽ bị xoá, <b>không</b> xoá ảnh/video/audio liên quan.
            </div>
          </button>

          <button
            onClick={onConfirmWithMedia}
            className="w-full text-left rounded-lg border px-3 py-2 hover:bg-red-50"
            title="Xoá cả bài và toàn bộ media liên quan"
          >
            <div className="font-medium text-sm text-red-700">Xoá bài + toàn bộ media</div>
            <div className="text-xs text-red-600">
              Xoá bài viết và <b>mọi</b> file trong group <code>post:&lt;id&gt;</code> (bao gồm ảnh
              bìa, ảnh/video/audio trong gallery).
            </div>
          </button>
        </div>

        <div className="mt-4 flex items-center justify-end gap-2">
          <button onClick={onClose} className="px-3 py-2 rounded-lg border hover:bg-gray-50 text-sm">
            Huỷ
          </button>
        </div>
      </div>
...
export default function AdminPostsPage() {
  ...
  // Mobile: ẩn/hiện filter
  const [filtersOpen, setFiltersOpen] = useState(false);

  // Modal xoá
  const [deleteTarget, setDeleteTarget] = useState(null); // post row
  ...
  const confirmDeleteWithMedia = async () => {
    ...
    try {
      ...
      // 2) Lấy & xoá toàn bộ files theo group (tối đa 1000)
      try {
        const list = await listFiles({ group, page: 1, limit: 1000, sort: "-order" });
        const files = list?.items || [];
        for (const f of files) {
          try {
            await deleteFile({ id: f._id });
          } catch {
            /* ignore per-file delete error */
          }
        }
      } catch {
        // BE chưa hỗ trợ group -> bỏ qua
      }

      // 2.2) Ảnh bìa (nếu không nằm trong group)
      if (doc?.coverFile?._id) {
        try {
          await deleteFile({ id: doc.coverFile._id });
        } catch {
          /* ignore */
        }
      }
  ...
  return (
    <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
          <div className="relative flex-1">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  reload(1); // Enter -> tìm ngay
                }
              }}
              placeholder="Tìm tiêu đề / slug / tác giả...  (mẹo: tag:seo  category:tai-lieu  author:6520fa...  slug:wfour)"
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 sm:flex-none inline-flex justify-center rounded-lg border px-3 py-2 text-sm hover:bg-gray-50"
        ...
        {/* Tổng số (ẩn trên header mobile khi filter mở) */}
      ...
      <div className="space-y-2">
        <div className="sm:hidden text-xs text-gray-500">
          💡 Bảng rộng — hãy <b>kéo ngang</b> để xem đủ thông tin.
        </div>
        <PostTable
          rows={items}
          page={page}
          ...
          // ❌ KHÔNG truyền query => PostTable không highlight nữa
        />
      </div>
